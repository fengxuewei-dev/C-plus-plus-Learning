...
/*
    ...
    拷贝构造函数： const Test& te1; 操作符重载 也是 const Test7 
        当想要使用一个类型的对象为另外一个该类型的对象来赋值的时候，就要定义拷贝构造函数

        class t2(t1); class t2 = t1; // 调用拷贝构造函数
        class t3; t3 = t2; // 调用 = 操作符重载

        当把一个对象当做实参传递给某个函数，若不是引用或指针，则调用的是该类的 拷贝构造函数
        同样，当返回一个对象，该对象会执行拷贝构造函数来进行赋值
*/
...
void test04(){
    ...
    Test t1 = func2();  // 在 func2() 函数中返回值的时候，有一个匿名对象 = return 的对象，调用拷贝构造函数，这个时候，fun2() 就是该匿名对象，这里不会再一次进行拷贝函数处理了，因为没有意义，所以这里主要是将 该匿名对象进行转正.
    cout << "test04() end...." << endl;

}
void test05(){
    cout << "test04() begin...." << endl;
    Test t1; // 这里的 t1 已经倍定义完成，所以，下面的匿名对象并没有转正，而是执行 operator= 之后，就释放掉了
    t1 = func2();  // 在 func2() 函数中返回值的时候，有一个匿名对象 = return 的对象，调用拷贝构造函数，这个时候，fun2() 就是该匿名对象，这里不会再一次进行拷贝函数处理了，因为没有意义，所以这里主要是将 该匿名对象进行转正.
    cout << "test04() end...." << endl;
